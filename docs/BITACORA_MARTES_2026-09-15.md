...
**Archivo instalable de Android.** Se compiló el archivo instalable en modo de publicación, apuntando al servicio desplegado, con una opción propia en el panel de control. El archivo resultante ocupa 53,8 MB y se instala en Android 8.0 o superior. La compilación descartó los recursos no utilizados —el paquete de iconos se redujo un 99,4 %— y dejó dos constancias: los complementos `firebase_auth` y `firebase_core` aplican hoy el complemento Gradle de Kotlin de forma directa, migración que una versión próxima de Flutter pedirá, y el archivo se firma con la clave de depuración, de modo que la firma con un almacén propio queda como tarea previa a cualquier distribución fuera del ámbito académico.
