...
| Presupuesto de páginas | Los anexos **no cuentan** para el límite de 30 a 40 páginas, así que la documentación de soporte puede ser exhaustiva. **El cuerpo, en cambio, excede el límite y la brecha creció**: medido el 14/09, tiene 15 738 palabras (unas 42 páginas de texto) y, con las 8 tablas, los 2 fragmentos de código y las figuras que **ya existen** —4 diagramas y 9 bocetos, más capturas pendientes—, llegaría a 49 o 51 páginas. El plan de recorte de seis medidas (1 700 a 1 800 palabras) **ya no alcanza**: haría falta recortar del orden de 2 500 o trasladar contenido a los anexos. Se agregaron tres medidas más al plan, que está escrito dentro del borrador |
...
| Apartado 3.1 Conclusiones | **Reescrito**: las seis conclusiones citan su objetivo específico, su grado de cumplimiento y su evidencia concreta (apartados, tablas y cifras de pruebas). Los cinco marcadores vagos que había —«[resultado de las pruebas funcionales ejecutadas]», «[indicar resultados]», «[plataforma de alojamiento]»— se redujeron a **dos marcadores precisos**, que dependen de la ejecución de los casos de prueba y del despliegue |
| Recuento de marcadores | Corregido en la auditoría: son **55**, no 34. La primera estimación no contemplaba la columna de estado de la tabla de casos de prueba, que suma 23 más. Desplegar y ejecutar los casos cierra **51 de los 55** |
| Conversión a Word (verificada el 14/09) | Se probó el camino de entrega y estaba **roto**: `md2docx.py` genera tablas sin el elemento obligatorio `w:tblGrid`, que Word no puede interpretar, y no aplica estilos de título. El archivo de Word que existía tenía sus 5 tablas ilegibles y ningún estilo. El conversor del repositorio (`scripts/md_a_docx.py`) producía tablas válidas y estilos, pero en Calibri 11 con títulos en verde. Se le agregó la opción **`--institucional`**: Arial 12, interlineado 1,5, márgenes 4/3/3/3 cm, papel carta y la jerarquía del índice oficial (Título 1 = capítulo, 2 = apartado, 3 = subapartado), de modo que el índice y la numeración se generen solos. Verificado: 8 de 8 tablas legibles y jerarquía coincidente con el índice |
| Bocetos de la interfaz | **Nueve figuras generadas** en `Figuras/` con el script `generar_figuras_interfaz.py`: las siete pantallas, la administración de usuarios y —la más útil— los cuatro estados de una vista que consume datos, que es la parte del diseño que el módulo pide documentar y que suele improvisarse. Verificadas por geometría: el texto entra en todos los contenedores (el más ancho usa 400 de 824 píxeles) y el contenido queda centrado entre la cabecera y la barra inferior |
| Criterios de accesibilidad | Agregados al apartado 2.4.3: el estado no se comunica solo por color sino con etiqueta de texto y límites a la vista, las acciones se identifican con texto y no solo con iconos, y los cuatro estados se anuncian con texto |
| Diagramas técnicos | **Tres figuras generadas** con `generar_figuras_diagramas.py`, sin depender de renderizarlas en un sitio externo: la arquitectura de componentes (2.4.1), los casos de uso (2.3.4) y —la más útil para la defensa— la **secuencia del registro de una lectura** (2.4.4 y 2.6), que muestra dónde se valida, dónde se decide y qué se devuelve cuando algo falla. Con los nueve bocetos, el juego queda en 12 archivos, todos verificados por geometría |
| Figuras: total | 12 imágenes en `Figuras/` (9 bocetos de pantalla y 3 diagramas técnicos), unos 550 KB, más el diagrama de antecedentes, que sigue existiendo solo como código Mermaid. **Ya insertadas**: el conversor reconoce la sintaxis de imagen de Markdown y coloca cada figura centrada con su epígrafe debajo. Cinco van en el cuerpo (casos de uso, arquitectura, panel, los cuatro estados y la secuencia) y siete en el Anexo E como Figuras E.1 a E.7 |
| Numeración de figuras | Asignada y verificada: Figuras 1 a 5 en el cuerpo, 6 y 7 para los fragmentos de código del apartado 2.6.4 y E.1 a E.7 en el anexo. **No queda ningún marcador `[N]`** en el documento |
| Preliminares y formato institucional | **Generados por el conversor** con la opción `--institucional`: portada, contratapa, hoja de aprobación, hoja de advertencia (con el texto oficial) y los tres índices como **campos de Word**, que se actualizan solos tras cada corrección. Además: márgenes 4/3/3/3 cm, papel carta, Arial 12 con interlineado 1,5, títulos en negro con la jerarquía del índice oficial, **numeración romana en los preliminares y arábiga desde el Capítulo 1** con el número en el pie derecho, títulos de figura arriba a la izquierda en 10 puntos con la fuente debajo, y **tablas sin líneas verticales** (las tres últimas, exigencias expresas de la norma de presentación) |
| Contradicción de formato, resuelta | El módulo dice «las figuras llevan epígrafe debajo»; la plantilla oficial y las normas de presentación que cita dicen que **el título va arriba, al margen izquierdo, en 10 puntos y la fuente debajo**. Se siguió la plantilla porque el propio módulo dispone que *el Formato 2020 manda en lo formal y los Lineamientos en lo técnico*. **Conviene confirmarlo con el tutor el martes**: es una pregunta de treinta segundos y el conversor puede cambiarlo en una línea |
| Defecto grave corregido: notas internas | El conversor **no filtraba las notas internas** del borrador, de modo que **las 31 notas se imprimían como texto** en el documento de Word: las de revisión, la de ensamble, la de presupuesto de extensión —con las páginas estimadas y el plan de recorte— y las que señalan lo que falta. El defecto era invisible en el Markdown (donde los comentarios no se ven) y habría aparecido recién al imprimir. Corregido: el conversor elimina los comentarios antes de procesar. Verificado: **cero notas** en el documento generado |
| Presentación del cuerpo | Los separadores `---` del borrador se dibujaban como líneas de guiones bajos y cada capítulo seguía al anterior sin salto: ahora **se omiten** y **cada capítulo y la bibliografía comienzan en página nueva**, como corresponde. El cuerpo va **justificado**, igual que el resumen, sin afectar títulos, epígrafes ni celdas de tabla. Verificado: solo quedan las cuatro líneas de firma de la hoja de aprobación y 11 saltos de página |
| Resumen reescrito y ubicado | El resumen tenía dos problemas: no mencionaba el servicio de API y afirmaba que «las pruebas funcionales y de usabilidad evidenciaron el cumplimiento de los requisitos», que es un resultado todavía no ejecutado. Se reescribió con **290 palabras** (el máximo son 300), incorpora la metodología y el servicio de API, y afirma solo lo verificado. Además, el conversor ahora **lo extrae del cuerpo y lo coloca como última parte preliminar**, después de los índices, que es donde lo ubica el índice oficial |
| Pendiente en Word (no automatizable) | Pegar el **escudo de la universidad** en el lugar reservado de la portada; completar los **nombres del tribunal** en la hoja de aprobación; decidir si se incluyen **dedicatoria y agradecimientos** (opcionales); y revisar que el **nombre completo** del autor sea el correcto en la portada. Los índices y el resumen ya no requieren trabajo manual |
